...
public interface FoodInfoRepository extends JpaRepository<FoodInfo, Integer> {
    ...
    @Query(nativeQuery=true, value="SELECT f.id, f.food_category_id, f.name, f.standard_expire_day, f.standard_price, f.standard_volume, f.storage_type, f.custom_bool, f.season_spring, f.season_summer, f.season_winter, fc.name as category_name FROM FOOD_INFO f inner join FOOD_CATEGORY as fc on f.food_category_id = fc.id WHERE f.food_category_id =:bno")
    // @Query(
    ...
    @Query(nativeQuery=true, value="SELECT f.id, f.image, f.food_category_id, f.name, f.standard_expire_day, f.standard_price, f.standard_volume, f.storage_type, f.custom_bool, f.season_spring, f.season_summer, f.season_winter, fc.name as category_name FROM FOOD_INFO f inner join FOOD_CATEGORY as fc on f.food_category_id = fc.id WHERE f.food_category_id =:foodCategoryId")
    List<Food> findAllFoodByCategoryId(@Param("foodCategoryId") int foodCategoryId);
